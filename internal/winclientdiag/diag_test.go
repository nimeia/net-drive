...
func TestCheckerRunAndExport(t *testing.T) {
	...
	for _, want := range []string{"report.txt", "report.json", "explorer-smoke.md", "explorer-smoke.json", "explorer-request-matrix.md", "explorer-request-matrix.json", "winfsp-native-callbacks.md", "winfsp-native-callbacks.json", "recovery.json", "log-tail.txt", "windows-host-validation-template.md", "windows-host-validation-template.json", "windows-host-validation-result-template.md", "windows-host-validation-result-template.json"} {
		if !have[want] {
