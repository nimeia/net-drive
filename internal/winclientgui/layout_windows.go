...
func (a *app) initControls() {
	...
	a.addStaticCommon(idHeaderStatus, "Runtime idle", 264, headerY, 804, rowH)
...
func (a *app) initDashboardControls(y int) {
	...
	a.addLabel(pageDashboard, "Mount runtime status and quick actions", 16, y+28, 360, 20)
	...
	a.addOutput(pageDashboard, idDashboardSummary, 16, y+108, 1050, 700)
}
...
func (a *app) initProfileControls(y int) {
	...
	a.addEdit(pageProfiles, idLocalPath, xInput, y, 880, rowH)
	y += rowH + gap
...
func (a *app) initDiagnosticsControls(y int) {
	...
	a.addLabel(pageDiagnostics, "Advanced smoke actions over the current in-memory profile", 16, y+28, 420, 20)
	y += 56
	...
	a.addButton(pageDiagnostics, idClear, "Clear Output", 644, y-2, 110, 28)
	y += 40

	a.addOutput(pageDiagnostics, idDiagnosticsSummary, 16, y, 1050, 180)
	y += 196
	a.addOutput(pageDiagnostics, idOutput, 16, y, 1050, 520)
}
