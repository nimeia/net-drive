...
func TestManifestMarkdownChecklist(t *testing.T) {
	...
	for _, want := range []string{"windows release validation", "1.2.3", "native callback", "explorer smoke", "backfill windows-host-validation-result-template.json", "backfill patch"} {
