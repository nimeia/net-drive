...
func DefaultExplorerSmoke() []Scenario {
	return []Scenario{
		...
		{ID: "explorer-root-browse", Name: "Browse root directory", Goal: "Verify Explorer can enumerate the root directory.", Steps: []string{"Open the mounted root.", "Sort by name and refresh once."}, Expected: []string{"Explorer lists entries without hanging.", "Refresh does not produce a binding/runtime error."}},
