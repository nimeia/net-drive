...
func DefaultNativeCallbackTable(binding BindingInfo) NativeCallbackTable {
	...
	callbacks := []NativeCallback{
		...
		{Name: "GetFileInfo", State: state, ExplorerHot: true, Detail: "Path metadata lookup is bridged through adapter -> mountcore.", RequiredFor: []string{"explorer-root-browse", "explorer-file-preview"}},
		...
		{Name: "Cleanup", State: CallbackStateGap, ExplorerHot: true, Detail: "Explicit native cleanup callback is not bridged yet.", Remediation: "Complete the native callback table bridge for Cleanup before claiming full Explorer lifecycle coverage.", RequiredFor: []string{"explorer-unmount-cleanup"}},
		{Name: "Flush", State: CallbackStateGap, ExplorerHot: false, Detail: "Flush callback is not required for read-only MVP, but Explorer may still issue it.", Remediation: "Map Flush to a harmless read-only success path in the native bridge."},
		{Name: "GetSecurityByName", State: CallbackStateGap, ExplorerHot: true, Detail: "Security descriptor probing is not bridged yet.", Remediation: "Provide a minimal security descriptor callback for richer Explorer metadata flows."},
		{Name: "GetSecurity", State: CallbackStateGap, ExplorerHot: false, Detail: "Per-handle security query is not bridged yet.", Remediation: "Add security descriptor translation once native callback marshaling is complete."},
