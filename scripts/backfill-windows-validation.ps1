...
if ($PatchJson) {
  ...
  $patch = Get-Content $PatchJson -Raw | ConvertFrom-Json -Depth 16
...
if ($InstallerResultsJson) {
  if (-not (Test-Path $InstallerResultsJson)) { throw "Missing installer results json: $InstallerResultsJson" }
  $installer = Get-Content $InstallerResultsJson -Raw | ConvertFrom-Json -Depth 16
  if ($installer.log_dir) { $validation.environment.installer_log_dir = $installer.log_dir }
  foreach ($pair in @(
    @{ channel='msi'; action='install'; result=$installer.msi.install; checklist='MSI install succeeded' },
    @{ channel='msi'; action='upgrade'; result=$installer.msi.upgrade; checklist='' },
    @{ channel='msi'; action='uninstall'; result=$installer.msi.uninstall; checklist='Uninstall removes binaries and leaves expected user data policy' },
    @{ channel='exe'; action='portable-launch'; result=$installer.exe.portable_launch; checklist='EXE/portable bundle launch succeeded' }
  )) {
    $result = $pair.result
    if (-not $result) { continue }
    $hit = $validation.installer_runs | Where-Object { $_.channel -eq $pair.channel -and $_.action -eq $pair.action } | Select-Object -First 1
    if ($hit) {
      $hit.status = $result.status
      $hit.notes = $result.notes
      if ($result.version_from) { $hit.version_from = $result.version_from }
      if ($result.version_to) { $hit.version_to = $result.version_to }
      if ($result.log_path) { $hit.log_path = $result.log_path }
    }
    if ($pair.checklist) {
      $check = $validation.installer_checklist | Where-Object { $_.item -eq $pair.checklist } | Select-Object -First 1
      if ($check) { $check.status = $result.status; $check.notes = $result.notes }
    }
  }
  if ($installer.notes) {
    if (-not $validation.notes) { $validation | Add-Member -NotePropertyName notes -NotePropertyValue @() -Force }
    $validation.notes += $installer.notes
  }
}
...
$completedByValue = if ($CompletedBy) { $CompletedBy } elseif ($validation.completed_by) { $validation.completed_by } else { '' }
...
$validation | ConvertTo-Json -Depth 16 | Set-Content -Encoding UTF8 $OutJson
@"
...
Use finalize-windows-release.ps1 to regenerate closure, issue-list, fix-plan, and RC outputs.
